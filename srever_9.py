...
while True:
    ...
    for sock in players[:]: #так можно безопасно удалять элементы из списка
                            # внутри цикла. Без этого при удалении будет RuntimeError
        try:
    ...
    time.sleep(1) # ограничение скорости можно сделать (0.016) чтобы не тормозить игру
